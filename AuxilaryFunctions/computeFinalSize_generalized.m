...
A=R0*C;
s=1-r-v_protected-infected;
s_nv=1-r-v_protected-v_vunerable-infected;

[x,fval,exitflag,output]=fsolve(@(z) g(z,A,r,v_protected,infected,betaVac),1+0*s,optimset('Display','none','TolX',1e-8));
if exitflag<0
    [x,fval,exitflag,output]=fsolve(@(z) g(z,A,r,v_protected,infected,betaVac),1+0*s,optimset('Display','iter','TolX',1e-8));
    display(['*!!* ',num2str(R0),num2str(betaVac)])
    keyboard
end
r_inf=real(x);
